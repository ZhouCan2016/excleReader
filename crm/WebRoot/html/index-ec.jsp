...
<jsp:directive.page import="com.team3.entity.SysRight"/>
...
function initialise() {
	...
	<%	
		...
		while(it.hasNext()){
		SysRight r=(SysRight)it.next();
		list1.add(r);
	}
	for(int j=1;j<7;j++){
		for(int i=0;i<list1.size();i++){
			SysRight r=(SysRight)list1.get(i);
			if(r.getRightCode().equals("L0"+j))
			{
				out.print(r.getRightCode()+"= theMenu.addChild("+r.getRightParentCode()+",'"+r.getRightType()+"','"+r.getRightText()+"','"+"mywork.htm"+"','"+r.getRightTip()+"');\n");
				list1.remove(r);
				break;
			}
		}
	...
	for(int j=1;j<6;j++){
		for(int i=0;i<list1.size();i++){
			SysRight r=(SysRight)list1.get(i);
			if(r.getRightCode().substring(4).equals(""+j))
			{
			out.print(r.getRightCode()+"= theMenu.addChild("+r.getRightParentCode()+",'"+r.getRightType()+"','"+r.getRightText()+"','"+"/crm/"+r.getRightUrl()+"','"+r.getRightTip()+"');\n");
			}
		}
	}
	%>
